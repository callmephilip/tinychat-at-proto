...
import { DeleteMembershipRecord, DeleteServerRecord, NewMembershipRecord, NewServerRecord } from 'tinychat/firehose.ts';
import { ServerSummaryView, ServerView, validateServerSummaryView, validateServerView } from 'tinychat/api/types/chat/tinychat/server/defs.ts';
...
export class Servers {
  ...
  public createMembership(m: NewMembershipRecord) {
    ...
    if (Deno.env.get("SEED_MESSAGES_AFTER_SERVER_CREATION")) {
      console.log("SEED_MESSAGES_AFTER_SERVER_CREATION is set, checking if we need to seed messages");
      const messageCount = this.db.prepare(`SELECT COUNT(*) FROM messages WHERE server = '${m.commit.record.server}'`).value()![0];

      console.log("Message count: ", messageCount, "for server: ", m.commit.record.server);
...
Deno.test("getServersForMember", () => {
  ...
  const db = testServers.db;
  const servers = new Servers(db);
  const serversForUser1 = servers.getServersForMember({ did: TestDatabase.user1 });
