...
const makeCID = async (d: object): Promise<string> => {
  const { cid } = await dataToCborBlock(d);
  return cid.toString();
};
...
export function fetchView<T>(
  ...
  return db.prepare(sql).all().map((r) => {
    ...
    const v = validate(r);
    if (!v.success) {
      console.error("Failed to validate view", v, r);
    }
    // @ts-ignore yolo
    return v.value;
  }).filter((v) => v);
...
export const seedMessages = async ({
  ...
  messagingSeed.split("\n").forEach((text, i) => {
    ...
    db.prepare(
      ...
      uri: `at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.message/${TID.nextStr()}`,
      cid,
      channel,
      server,
      record: JSON.stringify({ createdAt, server, channel, text: `[${i + 1}] ${text}` }),
      sender: user,
...
export class TestDatabase {
  ...
  public static server: string = "at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.server/3lgfm4edsy72b";
  public static server2: string = "at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.server/3lgfm4edsy767";
  ...
  public static async setup(
    ...
    [0, 1].forEach((i) => {
      ...
      `,
        ...
        .run({
          uri: `at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.membership/${TID.nextStr()}`,
    ...
    if (messages) {
      await seedMessages({ db: service.db, server: TestDatabase.server });
    }
  ...
  public async user1MessagesChannel1(
    ...
    const uri = `at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.message/${TID.nextStr()}`;
    this.receiveMessage({
...
import { assert, assertEquals, assertRejects, assertThrows } from "asserts";
// 🦕 AUTOGENERATED! DO NOT EDIT! File to edit: db.ipynb
...
Deno.test("getDatabase", () => {
  ...
  assert(
    ts.some((t) => t.name === "users"),
    "got users table",
  );
  assert(
    ts.some((t) => t.name === "servers"),
    "got servers table",
  );
  assert(
    ts.some((t) => t.name === "channels"),
    "got channels table",
  );
  assert(
  ...
  assert(
    ts.some((t) => t.name === "messages"),
    "got messages table",
  );
  assert(
    ts.some((t) => t.name === "read_receipts"),
    "got read_receipts table",
  );
  ...
  assert(
    views.some((v) => v.name === "message_view"),
    "got message_view",
  );
  assert(
...
Deno.test("test keys for memberships", () => {
  ...
  assertThrows(() => {
    db.prepare(
      "INSERT INTO server_memberships (uri, server, user) VALUES (:uri, :server, :user)",
    ).run({
      uri: `at://did:plc:ubdeopbbkbgedccgbum7dhsh/chat.tinychat.core.membership/${TID.nextStr()}`,
      server: "1",
      user: "2",
...
Deno.test("waitForSync", async () => {
  const db = getDatabase({ reset: true });
  // create table named foo
  const foo = "CREATE TABLE foo (id INTEGER PRIMARY KEY, name TEXT)";
  db.exec(foo);

  assertEquals(
    await waitForSync<string>({
      db,
      op: () => {
        db.exec("INSERT INTO foo (name) VALUES ('bar')");
        return Promise.resolve("bar");
      },
      sql: (s: string) => `SELECT * FROM foo WHERE name = '${s}'`,
    }),
    "bar",
    "works ok with a basic insert",
  );
  ...
  assertEquals(
    await waitForSync<string>({
      ...
      sql: (r) => `SELECT * FROM foo WHERE name = '${r}'`,
    }),
    "baz",
    "works with a little timeout < 5 seconds",
  );
  ...
  await assertRejects(
    () =>
      waitForSync<string>({
        db,
        op: () => Promise.resolve("faz"),
        sql: () => `SELECT * FROM foo WHERE name = 'faz'`,
      }),
...
Deno.test("TestDatabase", async () => {
  ...
  assert(tdb.db.prepare("SELECT * FROM messages").all().length > 0, "got test messages in");
