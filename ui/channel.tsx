...
export const ChannelLabel = ({ channel }: { channel: ChannelView }) => {
  return (
    <label
      class={"channel-label w-full cursor-pointer font-medium" +
        (hasUnreadMessages(channel) ? " has-new-messages" : "")}
      for={`tab-${channel.id}`}
      id={`nav-channel-label-${channel.id}`}
    >
      {channel.name}
    </label>
  );
